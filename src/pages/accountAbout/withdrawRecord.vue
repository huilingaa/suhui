<template>
  ...
  <view class="exchangelist">
    <my-navbar title="提币记录" leftIcon="back"></my-navbar>
    <my-empty v-if="isNoData" />
    <view class="uni-share-content-item" v-for="(item,index) in listData" :key="index">
      <view class="ListCell">
        <view class="topDiv">
          <view class="fromName">提币类型：{{item.fromAsset}}</view>
          <view class="arraw">2012.12.12 12:12:12</view>
        </view>
        <view class="topDiv">
          <view class="fromName">提币数量：{{item.feeQuantity}} USDT</view>
        </view>
        <view class="topDiv">
          <view class="fromName adr">{{item.adr}}</view>
